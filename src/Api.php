...
class Api {
    ...
    public function init(Request $request) {
        $this->createMemberValue($request);
     ...
     */
    protected function createMemberValue(Request $request) {

        $r = [];

        foreach ($this->getApiRules() as $key => $rule) {

            $value = DI()->request->getByRule($rule);
            $this->$key = $value;

            $r[$key] = $value;
        }

        $request->appendData($r);
    }
...
```
    ...
    public function jsonEcho($ret,$data = '',$msg){
        header('Content-type:text/json;charset=utf-8');
        echo json_encode(['ret'=>$ret,'data'=>$data,'msg'=>$msg]);die;
    }
}
