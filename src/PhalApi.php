...
use function App\dependencyInjection;
...
```
    ...
    public function response() {
        ...
        try {
            ...
            $data =  $this->middleware(function (Request $request) use ($api, $action){

               $args = [];
               dependencyInjection($api, $action, $args);

               return call_user_func_array(array($api, $action), $args);
           });
    ...
    protected function middleware(\Closure $closure)
    {
        $request = DI()->request;
       ...
       $next = array_reduce($middlware, function ($carray, $item) use ($request) {
           ...
           dependencyInjection($instace, 'handle', $di);


          return call_user_func_array([$instace, 'handle'], array_merge([$request, $carray], array_slice($di, 3)));
